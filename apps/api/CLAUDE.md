...
- `workers_dev` and `preview_urls` are `false`: the deployed Worker has no public URL. Only the web Worker (`apps/web`) reaches it, through its `API` service binding, so every request has already passed Cloudflare Access.
