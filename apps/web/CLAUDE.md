...
- Pages open in view mode and edit a draft. Each page (Home, Schedule, Shops, Info) holds its list in `useEditSession` (`src/components/editor/useEditSession.ts`), which keeps the saved data and a draft. `EditControls` shows 編輯 in view mode and 取消／完成 while editing, and only when `editable`. Every edit control (`EditBtn`, `DeleteBtn`, `AddBtn`, `EditModal`) is gated on `editable && editing` (and hidden while saving). `EditModal`'s 確定 only changes the draft.
