...
- Versions (#31): every save names the version its data was read at, and the API refuses it (412) when someone else saved in between. `loadTripData`'s `version` comes from the response's `ETag` (null for the static JSON); `loadTrips` returns `TripEntry`s, each with the `version` of its fields (0 for the static JSON). `saveTripData(tripId, type, data, version)` and `updateTrip(tripId, fields, version)` send it as `If-Match` and resolve with the new version (`updateTrip` with the whole `TripEntry`); `uploadCover` returns the trip's new version too. A refused save throws `ConflictError`. The trip pages keep their list's version in a ref, set on every load and after every save.
